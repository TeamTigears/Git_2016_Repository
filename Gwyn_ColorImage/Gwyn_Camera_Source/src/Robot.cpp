...
#define RINGLIGHT_I2C_DEVICE_ADDR 0x08
#define USI_DATA_REGISTER 0x0F /* to be changed later */
...
{
		ring_light_I2C_connection = new I2C(I2C::kOnboard, RINGLIGHT_I2C_DEVICE_ADDR>>1);
#ifndef USING_DIRECT_TALLON_CONTROL
...
private:
	...
	{
		bool i2c_status;
		uint8_t i2c_data[4] = {0x55,0x11,0x22,0x33};
//		uint8_t *dat = 0;
		//		unsigned char r_light_data[3] = {0xFF,0,0};
		i2c_status = ring_light_I2C_connection->WriteBulk(i2c_data,4);
//		ring_light_I2C_connection->Read(0x0F,2,dat);
		printf("I2C Write status?: %d\n",i2c_status);
//		printf("I2C Read value?:%d\n", (int)(dat));
//		printf("I2C Write abort?(red): %d\n",ring_light_I2C_connection->Write(USI_DATA_REGISTER,0x55));
//		printf("I2C Write abort?(red): %d\n",ring_light_I2C_connection->Write(USI_DATA_REGISTER,0x55));
//		printf("I2C Write abort?(green): %d\n",ring_light_I2C_connection->Write(USI_DATA_REGISTER,0x55));
//		printf("I2C Write abort?(blue): %d\n",ring_light_I2C_connection->Write(USI_DATA_REGISTER,0xDF>>1));
		...
		//CES: Allows user to switch between arcade and tank drive using the z-axis of the joystick
	...
	} //SwitchBoxHandler(Joystick *sw_box, Joystick *r_joystick, Joystick *l_joystick,
	//float *l_motor_power, float *r_motor_power,
	//Encoder *r_encoder, Encoder *l_encoder, int *r_encoder_target, int *l_encoder_target,
	//bool *maintain_position_vals_set, int *reverse_direction_value,
	//bool *auto_on_value, int *auto_mode_value, DoubleSolenoid::Value *rotate_command_value)
	...
	int GetSpeedStartPoint(float joystickY, int image_start_basis,ImageInfo* vis_info)
	{
		...
		if (joystickY > -0.35 && joystickY <= 1){
			return image_start_basis + 80*BYTES_PER_PIXEL*vis_info->xRes;
		}else if (joystickY <= -0.35 && joystickY > -0.50){
			return image_start_basis + 50*BYTES_PER_PIXEL*vis_info->xRes;
		}else if (joystickY <= -0.50 && joystickY > -0.75){
			return image_start_basis + 20*BYTES_PER_PIXEL*vis_info->xRes;
		}else if (joystickY <= -0.75 && joystickY >= -1.0){
			return image_start_basis + -10*BYTES_PER_PIXEL*vis_info->xRes;
		}else{
			return image_start_basis + 80*BYTES_PER_PIXEL*vis_info->xRes;
		}
	} //GetSpeedStartPoint(float joystickY, int image_start_basis,ImageInfo* vis_info)
		...
		if(gripper_infront_bot){
			if(timer.Get()-time_gripper_move >= VISION_DELAY_TIME){
				...
				if(right_joystick.GetRawButton(RJSTK_CAPTURE_CONSTANT)){
					...
					comparison_constant = green_test + 200;
